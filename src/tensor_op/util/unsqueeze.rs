...
#[derive(Clone)]
pub(crate) struct TensorUnSqueeze {
    dimension: usize,
}

impl TensorUnSqueeze {
    pub(crate) fn new(dimension: usize) -> Self {
        TensorUnSqueeze { dimension }
    }
}

impl<T, F> TensorOperation<T, F> for TensorUnSqueeze
where
    T: std::fmt::Debug
        + std::fmt::Display
        + NumCast
        ...
        match lhs_shape {
            Some(lhs_shape) => match rhs_shape {
                None => {
                    ...
                    lhs_shape.insert(self.dimension, 1_usize);
                    lhs_shape
                }
                _ => unreachable!(),
            },
